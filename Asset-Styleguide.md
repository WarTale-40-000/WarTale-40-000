...
- One base name, all variants. Every asset has a single canonical base name. All file variants (model, texture, inventory icon, HUD icon, crafting-category icon, item-definition JSON) derive from that base name using only a suffix or directory change. Nothing else changes.
- PascalCase_with_underscores. Words within a logical segment use PascalCase. Segments are separated by a single underscore. Example: `Cadian_Boltpistol`, `BloodPact_Flamer`.
- No spaces, no special characters, no lowercase-only. File names are case-sensitive on Linux (the target Docker environment). Always use the casing rules below.
- Category prefix on item-definition files. JSON item-definition files include a category prefix (`Weapon_`, `Armor_`, `Ammo_`, `Bench_`, `Ingredient_`) that mirrors the parent directory name. This allows the JSON file name to be unambiguous when viewed in isolation.
- Faction segment comes second. For faction-specific items the faction name is the second segment: `Weapon_BloodPact_Boltpistol`, `Armor_Cadian_Chest`. For faction-agnostic items the faction segment is omitted: `Weapon_Chainsword`.
...
| `Ammo_`       | All ammunition items                      | `Server/Item/Items/Ammunition/`                                          |
...
For every asset with base name `{BaseName}`, the following files must exist and use exactly the names shown. No other naming pattern is acceptable.
...
Note: The inventory icon and HUD icon share the same file name because they live in different directories (see section 5). They do not need a distinguishing suffix.

---

## 5. Directory Structure

The resource tree is rooted at `src/main/resources/`. The structure below is normative. All paths are relative to that root.
...
| File                 | Pattern                       | Example                 |
| -------------------- | ----------------------------- | ----------------------- |
| Item-definition JSON | `Ammo_{ItemName}.json`        | `Ammo_Bolt.json`        |
| Model                | `Ammo_{ItemName}.blockymodel` | `Ammo_Bolt.blockymodel` |
| Texture              | `Ammo_{ItemName}_Texture.png` | `Ammo_Bolt_Texture.png` |
| Inventory icon       | `Ammo_{ItemName}.png`         | `Ammo_Bolt.png`         |

The subfolder under `Ammunition/` takes the base name: `Ammo_Bolt/`, `Ammo_Auto/`, `Ammo_Laser/`.

Full example:

```
Server/Item/Items/Ammunition/Ammo_Bolt.json
Common/Resources/Items/Warhammer/Ammunition/Ammo_Bolt/Ammo_Bolt.blockymodel
Common/Resources/Items/Warhammer/Ammunition/Ammo_Bolt/Ammo_Bolt_Texture.png
Common/Icons/ItemsGenerated/Warhammer/Ammunition/Ammo_Bolt.png
```
...
Note: Ingredients may share a generic model (e.g. `Ingot.blockymodel`) stored under `Common/Resources/Items/Warhammer/Materials/`. Only the texture and icon are unique per ingredient.
...
Projectile textures do not use the `_Texture` suffix because they are referenced inside a flat model-entity definition rather than an item JSON.
...
Sound files are OGG and are referenced by string event IDs only, never by file path in item-definition JSON. The directory hierarchy is informational.
...
The `weaponId` string interpolated in `WartaleHUD.java` to build the HUD icon path must be the full base name of the weapon so the resolved path is consistent:
...
- [ ] If the item appears in a crafting bench category, `Common/Icons/CraftingCategories/{BenchBaseName}/{BaseName}.png` exists.
- [ ] All path fields in the JSON (`Model`, `Texture`, `Icon`, and any `Animation`) resolve to files that actually exist in the repository using the exact formats from section 6.
