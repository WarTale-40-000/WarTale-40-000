...
public class ShootInteraction extends SimpleInstantInteraction {
  ...
  public static final BuilderCodec<ShootInteraction> CODEC =
      BuilderCodec.builder(
              ShootInteraction.class, ShootInteraction::new, SimpleInstantInteraction.CODEC)
          .appendInherited(
              new KeyedCodec<>("ProjectileId", Codec.STRING),
              (obj, val) -> obj.projectileId = val,
              obj -> obj.projectileId,
              (obj, p) -> obj.projectileId = p.projectileId)
          .addValidator(Validators.nonNull())
          .addValidator(Projectile.VALIDATOR_CACHE.getValidator().late())
          .add()
          .appendInherited(
              new KeyedCodec<>("MaxSpreadAngle", Codec.FLOAT, false),
              (obj, val) -> obj.maxSpreadAngle = val,
              obj -> obj.maxSpreadAngle,
              (obj, p) -> obj.maxSpreadAngle = p.maxSpreadAngle)
          .add()
          .build();
  ...
  protected void firstRun(
    ...
    UUIDComponent sourceUuidComponent =
        commandBuffer.getComponent(sourceRef, UUIDComponent.getComponentType());
    if (sourceUuidComponent != null) {
      ...
      Holder<EntityStore> holder =
          ProjectileComponent.assembleDefaultProjectile(
              timeResource, this.projectileId, lookPosition, lookRotation);
      ProjectileComponent projectileComponent =
          holder.getComponent(ProjectileComponent.getComponentType());
