...
public final class ArmorVariantStore
    extends HytaleAssetStore<
        String, ArmorVariantConfig, IndexedLookupTableAssetMap<String, ArmorVariantConfig>> {
  ...
  private static void validateConfig(@Nonnull ArmorVariantConfig cfg) {
    ...
    for (String itemId : cfg.getItemIds()) {
      ...
      if (item == null) {
        LOGGER.atWarning().log("%s references missing item %s", cfg.getId(), itemId);
        continue;
      }

      if (item.getArmor() == null) {
        LOGGER.atWarning().log("%s references non-armor item %s", cfg.getId(), itemId);
      }
    }
  }
