...
public class KillSystem extends DeathSystems.OnDeathSystem {
    ...
    public void onComponentAdded(
        ...
        NPCEntity targetEntity = store.getComponent(ref, Objects.requireNonNull(NPCEntity.getComponentType()));
        if (targetEntity == null) return;

        // Check if target has entity level component
        EntityLevelComponent entityLevelComponent = store.getComponent(ref, EntityLevelComponent.getComponentType());
        ...
        GiveMasteryExperienceEvent.dispatch(killerRef, store, MasteryCalculations.getExperienceFromDefeatedLevel(entityLevelComponent.getLevel()), expectedMasteryComponentType);
    }
